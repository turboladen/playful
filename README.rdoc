...
#discovery_responses, which is an EventMachine::Queue.  This lets you pop off
the responses and do what you want with them (most likely you'll want to create
...
receive them.
  ...
  EM.run do
    ...
    device_controller.callback do |device|
      p device.services.first.class                 # UPnP::ControlPoint::Service
      p device.services.first.service_type          # "urn:schemas-upnp-org:service:ContentDirectory:1"

      # SOAP actions are converted to Ruby methods--show those
      p device.services.first.singleton_methods     # [:GetSystemUpdateID, :Search, :GetSearchCapabilities, :GetSortCapabilities, :Browse]

      # Call a SOAP method defined in the service.  This is extracted from the
      ...
      p device.services.first.GetSystemUpdateID     # { :Id => 1 }
    end

    # Note that you don't have to check for items in the Queue or for when the
    # Queue is empty: EventMachine will pop objects off the Queue as soon as
    ...
    searcher.discovery_responses.pop do |notification|
      ...
      # need to device callback and errback blocks to handle when the Device
